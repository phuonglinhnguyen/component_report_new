...
export const initDataDashboard = (resources) => {
  let user = resources['user']
...
export const changeGroup = (id, data) => (dispatch, getState) => {
  let { user, group, project, dashboard } = getState().resources
  ...
  dispatch(crudGetList('project', { group_id: id },
    {
      onSuccess: {
        // notification: {
        //   body: 'dgs.notification.get_list_success',
        //   level: 'info'
        // }
      },
      onFailure:{}
...
export const changeProject = (id) => (dispatch, getState) => {
  ...
  if (id) {
    dispatch({
      type: DASHBROAD_CHANGE_VALUE_ITEM,
      payload: { project_id: id, edit: project.data[id] },
      meta:
      {
        resource: 'dashboard',
      }
    ...
    dispatch({
      type: DASHBROAD_CHANGE_VALUE_ITEM,
      payload: { project_id: id, edit: undefined },
      meta:
      {
        resource: 'dashboard',
      }
...
export const showCreateProject = () => (dispatch, getState) => {
  let { dashboard } = getState().resources;
...
export const onSubmit = (type) => (dispatch, getState) => {
  let { dashboard, group, project } = getState().resources;
  switch (type) {
    ...
    case DASHBROAD_DIALOG_TYPE_EDIT_PROJECT:
      {
        ...
        let body = { ...data.edit,group_id:data.edit.group_id==='all'?null:data.edit.group_id}
    ...
    case DASHBROAD_DIALOG_TYPE_CREATE_PROJECT:
      {
        let data = getDataObject('data', dashboard);
        let body = { ...data.add_project, active: true,group_id:data.add_project.group_id==='all'?null:data.add_project.group_id}
