...
import { getInstancesDetail } from '../redux/actionCreators';
...
const DetailCapture = (props) => {
	const { classes, step, task, projectId } = props;
	console.log({ task });
	...
	useState(() => {
		props.getInstancesDetail(projectId, definition_id, instance_id, taskId);
	});
	console.log({ step });
	...
	const renderDetail = () => {
		return userNameData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item, index) => {
			return (
				<TableRow>
